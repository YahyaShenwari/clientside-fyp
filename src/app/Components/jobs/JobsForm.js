...
const JobsForm = ({
  ...
  const handleDelete = (i) => {
    setFieldValue(
      "qualifications",
      values.qualifications.filter((v, index) => index !== i)
    );
  };
    ...
    <Fragment>
      <div className="row">
        ...
        <div className="form-group col-12">
          ...
          <span className="form-text text-muted">
            Please provide job description at least ({values.description.length}
            /200)
          </span>
        </div>
        <div className="form-group col-6">
        ...
        <div className="form-group col-12">
          {values.qualifications.map((value, i) => (
            <Chip
              ...
              variant="outlined"
              size="small"
              label={qualifications.filter((f) => f.code === value)[0].title}
      ...
      <div className=" d-flex flex-row justify-content-end">
        ...
        >
          {history.location.pathname.includes("/jobs/edit") ? "Save " : "Post "}{" "}
          Job
        </button>
      </div>
    </Fragment>
